<script lang="ts">
	import { flatPages } from '$lib/config/navigation';
	import { ArrowLeft, ArrowRight } from '@lucide/svelte';

	let { pathname }: { pathname: string } = $props();

	const currentIndex = $derived(flatPages.findIndex((p) => p.href === pathname));
	const prev = $derived(currentIndex > 0 ? flatPages[currentIndex - 1] : null);
	const next = $derived(currentIndex >= 0 && currentIndex < flatPages.length - 1 ? flatPages[currentIndex + 1] : null);
</script>
